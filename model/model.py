...
default_datasets = {
    ...
    # "a": "data/datasets/PersonenMetGebruikZVWZorgVoorZintuiglijkGehandicapten2019.json",
}
...
def give_advice(industry: str, prediction_sets: list):
    ...
    for i, rank in enumerate(ranking, 1):
        print(f"{i}. {rank} ({round(data[rank], 3)})")
    ...
    for serie in industry_series:
        ...
        for age in range(age_range[0], age_range[1]):
            ...
            try:
                ...
                for _ in range(round(0.01 * male_prop * people_in_pop)):
                    prediction_list.append(male_pred)
                for _ in range(round(0.01 * female_prop * people_in_pop)):
                    prediction_list.append(female_pred)
            except:
                if print_warnings:
                    print(
